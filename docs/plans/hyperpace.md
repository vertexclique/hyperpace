...
| Phase 8 (packaging) | done for Linux, configuration only for Windows/macOS | deb, rpm, appimage and the Arch package are built and, for the AppImage, actually launched (see the "Built packages" gap below for the full picture, including the display-stack fix this pass added); windows and macos remain configuration only, no such machine here |
...
| Device layer on real hardware | Compile-checked and simulator-tested only; never opened against a device | partly closed on 2026-09-16, when the operator authorized real-device use. Two examples in `hyperpace-device` were run by hand against the attached device: `probe` enumerated its collections and found the configuration channel at usage page `0xff02`, usage `0x0002`, exactly where `docs/research/mouse-protocol-v2.md` places it, alongside the `0xff06` firmware channel (report 9, 48 bytes) and the `0xff04` feature channel; `read_state` opened it read-only and the device answered its handshake with cid 102, mid 1 and an 8000 Hz wireless link, then reported battery on the 5 second poll (90 percent, 4055 mV, discharging). So the open path, the collection matcher, the handshake, the settings walk, the battery decode and the model resolution are all confirmed against the hardware. Still unproven: every write path, the unplug mapping in `map_hid_error`, and firmware flashing, which stays unauthorized |
| macOS and Windows paths | Implemented per documentation, never executed on those systems | needs those machines |
| Built packages | Packaging configuration existed under `dist/`, but nothing had been built or run, so whether the app even launches on this machine's own display stack (current Mesa, an NVIDIA GPU) was unproven | closed for Linux, as far as this machine can prove without root: `deb` (17.51 MiB), `rpm` (11.56 MiB), the Arch package (11.97 MiB, a real `makepkg` build) and the AppImage (113.94 MiB) are all built from the current source tree (`cargo tauri build` for deb/rpm/appimage, `makepkg --nodeps` for Arch since cargo and npm are rustup/nvm managed here, not pacman packages, so pacman's own dependency check would refuse a build that otherwise succeeds; `--nodeps` skips only that check, nothing is installed). Only the AppImage needs no install step, so it is the one actually launched here: the default, unpruned build aborts with no window (`Failed to create GBM buffer of size 960x640: Invalid argument`, WebKitGTK's DMABUF renderer failing against this machine's NVIDIA driver, a different bug from the AMD or Intel Mesa one the two research reports name; their ten-library prune alone does not fix it here, confirmed by testing with and without the prune). The fix that actually works is in the app itself (`crates/hyperpace-app/src/gpu_workaround.rs`: detects an NVIDIA GPU on a Wayland or X11 session at startup and re-execs with `WEBKIT_DISABLE_DMABUF_RENDERER=1`, so every package format carries it, not only the AppImage); rebuilding with it compiled in, and applying the documented library prune as a repeatable step (`scripts/package-appimage.sh` / `make package-appimage`, not a manual sequence), produces an AppImage that launches with no manual step and renders the real UI, confirmed by a fresh launch, a clean startup log with no GBM errors, and a screenshot of the window. `deb`, `rpm` and the Arch package were not installed (needs sudo, never used here); only checked structurally (`ar`/`tar` archive listing against the bundle config, plus the real `makepkg` build for Arch). macOS and Windows remain unbuilt: no such machine here |
